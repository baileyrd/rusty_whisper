use rusty_whisper::{
    audio, decoder::Decoder, encoder, model, tensor::Tensor, tokenizer::Tokenizer,
};
use std::fs::File;
...
fn main() {
    ...
    let t = Instant::now();
    let (mel, n_frames) =
        audio::log_mel_spectrogram(&samples, &m.mel_filters, m.hparams.n_mels as usize);
    ...
    println!(
        "decoder: {:.1} ms/token",
        t.elapsed().as_secs_f32() * 1000.0 / (n + 1) as f32
    );
}
